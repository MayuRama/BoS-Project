...
const DealerTransactions: React.FC = () => {
  ...
  // For demo, show all transactions as dealer's own
  const allTx = transactions;

  const filtered = allTx.filter(tx => {
    const matchType = filterType === 'All' || tx.type === filterType;
    const matchStatus = filterStatus === 'All' || tx.status === filterStatus;
    const matchSearch = search === '' || tx.refNumber.toLowerCase().includes(search.toLowerCase()) || tx.customerWallet.includes(search);
  ...
  return (
    <div className="p-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Complete transaction history</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <input type="text" placeholder="Search ref # or wallet..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                {['Ref #', 'Type', 'Customer Wallet', 'Amount (USD)', 'Amount (SL)', 'Rate', 'Status', 'Timestamp', ''].map(h => (
            ...
            <tbody className="divide-y divide-gray-50">
              {paginated.map(tx => (
                <tr key={tx.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-gray-500">{tx.customerWallet}</td>
        ...
        <Modal isOpen={!!selectedTx} onClose={() => setSelectedTx(null)} title={`Transaction — ${selectedTx.refNumber}`} size="md">
          <div className="space-y-3">
            {[
              ...
              { label: 'Customer Wallet', value: <span className="font-mono">{selectedTx.customerWallet}</span> },
